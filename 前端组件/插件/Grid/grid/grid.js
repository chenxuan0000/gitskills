...
gj.grid.configuration = {
    base: {
        ...
        style: {
            ...
            header: {
                cell: "gj-grid-header gj-grid-ui-thead-th",
                sortable: "gj-grid-sort",
                sorticon: "gj-grid-sort-icon"
            },
            content: {
                row: "gj-grid-row-default",
                rowHover: "gj-grid-row-hover",
                rowSelected: "gj-grid-row-active"
            },
            pager: {
                cell: "gj-grid-pager-cell",
                stateDisabled: "gj-grid-pager-disabled"
            },
            //TODO: move to expand/collapse plugin
            expandIcon: "gj-grid-icon gj-grid-icon-plus",
            collapseIcon: "gj-grid-icon gj-grid-icon-minus"
        },
    ...
    'configuration': {
        pager: {
            ...
            /** Array that contains a list with jquery objects that are going to be used on the left side of the pager.
                ...
                */
            rightControls: [
                $('<span title="首页" data-role="page-first" class="gj-pager-nav">首页</span>'),
                $('<span title="上一页" data-role="page-previous" class="gj-pager-nav">上一页</span>'),
                $('<span title="下一页" data-role="page-next" class="gj-pager-nav">下一页</span>'),
                $('<span title="尾页" data-role="page-last" class="gj-pager-nav">尾页</span>'),
            ],
    ...
    'private': {
        init: function ($grid) {
            ...
            if (data.pager && data.pager.enable) {
                ...
                $leftPanel = $('<div />').css({ 'float': 'left', 'margin-top': '10px' });
                $rightPanel = $('<div />').css({ 'float': 'right', 'margin-top': '10px' });
...
gj.grid.private = {
    ...
    InitGrid: function ($grid) {
        ...
        if ("checkbox" === data.selectionMethod) {
            data.columns = [{ title: '', field: data.dataKey, width: 24, align: 'center', type: 'checkbox' }].concat(data.columns);
        }
        $grid.append($("<tbody/>"));
    },
    ...
    HeaderRenderer: function ($grid) {
        ...
        for (i = 0; i < columns.length; i += 1) {
            ...
            $cell.css('text-align', columns[i].halign || 'left');
            ...
            if ('checkbox' === data.selectionMethod && 'multiple' === data.selectionType && 'checkbox' === columns[i].type) {
                ...
                $cell.append($('<div/>').text(typeof (columns[i].title) === 'undefined' ? columns[i].field : columns[i].title));
            }
            if (columns[i].sortable) {
                var $sortIcon = $('<span data-role="sorticon"/>').addClass(style.sorticon);
                $cell.addClass(style.sortable).append($sortIcon);
    ...
    CreateSortHandler: function ($grid, $cell) {
        return function () {
            ...
            if ($grid.count() > 0) {
                ...
                style = data.style.header;
                $cell.siblings().find('span[data-role="sorticon"]').removeClass("asc desc");
                $sortIcon = $cell.children('span[data-role="sorticon"]').removeClass("asc desc");
                $sortIcon.addClass('asc' === cellData.direction ? "asc" : "desc");
    ...
    StartLoading: function ($grid) {
        ...
        $loading = $("<div data-role='loading-text'>加载中...</div>").addClass(data.style.loadingText);
    ...
    RowRenderer: function ($grid, $row, record, position) {
        ...
        $row.addClass("gj-grid-row-default");
        if ($row.index() % 2 != 0) {
            $row.addClass("even");
        }
